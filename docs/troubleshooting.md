...
**Symptom**: `create-catalog` fails saying no searchable text found.
...
### Catalog creation hangs or times out
**Symptom**: `create-catalog` runs for a very long time.

**Solutions**:
1. **Large book**: Books with 1000+ pages take longer. Use `-v` to see progress:
   ```bash
   ia-utils -v create-catalog <id> -d ./catalogs/
   ```
...
3. **Use default mode**: Don't use `--full` unless you need hOCR features:
   ...
   # Slow (only if needed)
   ia-utils create-catalog <id> -d ./catalogs/ --full
   ```
...
**Symptom**: `search-catalog` returns nothing for terms you know exist.

**Solutions**:
1. **Check spelling**: OCR may have errors. Try variations:
   ```bash
   ia-utils search-catalog -c catalog.sqlite -q "femur"
   ia-utils search-catalog -c catalog.sqlite -q "femnr"  # OCR error
   ia-utils search-catalog -c catalog.sqlite -q "fem*"   # Prefix search
   ```

2. **Try simpler terms**: Break up complex queries:
   ```bash
   # Instead of
   ia-utils search-catalog -c catalog.sqlite -q "anterior cruciate ligament"
   # Try
   ia-utils search-catalog -c catalog.sqlite -q "cruciate"
   ```

3. **Check the catalog has content**:
   ```bash
...
4. **Hyphenated terms**: By default these are auto-quoted. Use `--raw` if needed:
   ```bash
   ia-utils search-catalog -c catalog.sqlite -q "self-adjusting" --raw
   ```
...
**Solutions**:
1. **Check OCR confidence** (hOCR catalogs only):
   ```sql
...
2. **View the page directly**:
   ```bash
   ia-utils get-page -c catalog.sqlite -l <leaf> -o check.jpg
   ```
...
1. **Quote phrases**:
   ```bash
   ia-utils search-catalog -c catalog.sqlite -q '"exact phrase"'
   ```

2. **Use `--raw` carefully**: Only when you need FTS operators:
   ```bash
   ia-utils search-catalog -c catalog.sqlite -q "term1 AND term2" --raw
   ```
...
1. **Leaf vs book page confusion**:
   ...
   ```bash
   # Check the mapping
   sqlite3 catalog.sqlite "SELECT leaf_num, book_page_number FROM page_numbers WHERE leaf_num = 175"
   ```
...
2. **Try different size**:
   ```bash
   ia-utils get-page -c catalog.sqlite -l 175 --size medium
   ia-utils get-page -c catalog.sqlite -l 175 --size small
   ```
...
**Cause**: Another process is using the catalog.
...
**Cause**: Querying wrong catalog type or corrupted catalog.

**Solutions**:
1. **Check catalog mode**:
   ```bash
   sqlite3 catalog.sqlite "SELECT * FROM catalog_metadata"
   ```
...
### Catalog file is very large
**Symptom**: Catalog is unexpectedly large (100MB+).
...
### Slow catalog creation
1. **Avoid `--full`** unless needed
2. **Check disk space**: Catalog creation needs temp space
