...
Create a quick index and check OCR quality:
```bash
ia-utils create-index <best_candidate> -d ./test/
ia-utils search-index -i ./test/*.sqlite -q "common term"
ia-utils get-text -i ./test/*.sqlite -l 50
```
...
ia-utils search-index -i index.sqlite -q "contents" -l 10
ia-utils get-text -i index.sqlite -l 2-10
```

### Finding the Index
```bash
# Index is usually at the end
ia-utils search-index -i index.sqlite -q "index" -l 10
# Look for high leaf numbers in results
ia-utils get-text -i index.sqlite -l 937  # Example index page
```
...
# Or query directly
sqlite3 index.sqlite "SELECT leaf_num FROM page_numbers WHERE book_page_number = '145'"
```

### Leaf Number to Book Page
```bash
sqlite3 index.sqlite "SELECT book_page_number FROM page_numbers WHERE leaf_num = 175"
```

### Finding Roman Numeral Pages (Front Matter)
```bash
sqlite3 index.sqlite "SELECT leaf_num, book_page_number FROM page_numbers WHERE book_page_number LIKE '%I%' ORDER BY leaf_num LIMIT 30"
```
...
ia-utils get-page -i index.sqlite -l 175 --size original -o page175.jp2
```
...
ia-utils get-page -i index.sqlite -l 175 --cutoff 5 -o enhanced.jpg
```
...
# Range of pages
ia-utils get-text -i index.sqlite -l 100-120 -o chapter5.txt

# As JSON for processing
ia-utils get-text -i index.sqlite -l 100-120 --output-format json -o chapter5.json
```

### Download Full PDF
```bash
ia-utils get-pdf -i index.sqlite -o book.pdf
# Or directly by ID
...
ia-utils create-index b31362138 -d ./compare/
ia-utils create-index b29336211 -d ./compare/

# Compare OCR quality
ia-utils info ./compare/*.sqlite

# Sample the same page from each
ia-utils search-index -i ./compare/scan1.sqlite -q "femur" -l 3
ia-utils search-index -i ./compare/scan2.sqlite -q "femur" -l 3
```

Factors to compare:
- `block_count`: More blocks usually means better OCR segmentation
- OCR confidence (if using --full hOCR indexes)
...
# Build indexes for selected items
for id in item1 item2 item3; do
  ia-utils create-index $id -d ./corpus/
done

# Search across all indexes
for idx in ./corpus/*.sqlite; do
  echo "=== $idx ==="
  ia-utils search-index -i "$idx" -q "your search term" -l 5
done
```
...
3. **Verify OCR quality early**: Create index and test search before deep dive
