...
@click.option('--size', type=click.Choice(['small', 'medium', 'large', 'original']),
              default='original', help='Image size (default: original, ignored with --viewer)')
@click.pass_context
def get_url(ctx, identifier, leaf, book, catalog, viewer, size):
    """Get URL for a page image or viewer from Internet Archive.
    ...
    IMAGE SIZES (for image URL, ignored with --viewer):
